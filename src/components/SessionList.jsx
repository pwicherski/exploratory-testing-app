...
const SessionList = () => {
  ...
  const handleImportConfirm = () => {
    if (importedSessions.length > 0) {
      setSessions(prevSessions => {
        const updatedSessions = [
          ...
          ...importedSessions.map(session => ({
            ...
            notes: Array.isArray(session.notes) ? session.notes.map(note => ({
              ...note,
              id: `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
              type: note.type || 'Note',
      ...
      toast.success(`${importedSessions.length} sessions imported successfully with all notes and details`);
